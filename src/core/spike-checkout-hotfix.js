...
(function(){
  ...
  window.saveRemoteAddress=async function(addr){
    ...
    if(!maps&&!Number.isFinite(Number(addr.latitude)))throw new Error('أضف رابط Google Maps للموقع');
    let latitude=Number(addr.latitude),longitude=Number(addr.longitude);
    if(maps){const pos=await SpikeCommerce.req('/locations/resolve-google-maps',{method:'POST',auth:true,body:{google_maps_url:maps}});latitude=Number(pos.latitude);longitude=Number(pos.longitude)}
    const body={city_id:cityId,label:addr.type||addr.area||'home',recipient_name:state.userProfile?.name||'Customer',phone:addr.phone||state.userProfile?.phone||'',address_line:addr.details||'',latitude,longitude,is_active:!!state.addressFormDefault,google_maps_url:maps||undefined};
