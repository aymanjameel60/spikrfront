...
(function(){
  ...
  const abs=v=>{const s=String(v||'').trim();if(!s)return '';if(/^https?:\/\//i.test(s))return s;const m=s.match(/^\/?uploads\/([^/?#]+)$/i);if(m)return SUPABASE_PUBLIC+encodeURIComponent(m[1]);return typeof window.absoluteProductImage==='function'?window.absoluteProductImage(s):s};
  ...
  function body(s,full=false){
    ...
    if(s.content_type==='categories'){
      const list=s.items||[];
      return list.length?`<div class="categories drag-scroll">${list.map(x=>`<button class="cat" data-action="open-spike-category" data-id="${x.id}"><img src="${abs(x.image_url)||A+'placeholder-product.svg'}" onerror="this.onerror=null;this.src='${A}placeholder-product.svg'"><span>${esc(x.name)}</span></button>`).join('')}</div>`:'<div class="empty-ux">لا توجد فئات</div>'
    }
    if(s.content_type==='collections'){
      const list=(s.items||[]).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
      return list.length?`<div class="categories drag-scroll spike-collections-strip">${list.map(x=>`<button class="cat" data-action="open-spike-collection" data-destination-type="${esc(x.destination_type||'')}" data-destination-id="${esc(x.destination_id||'')}"><img src="${abs(x.image_url)||A+'placeholder-product.svg'}" alt="${esc(x.name||'مجموعة')}" onerror="this.onerror=null;this.src='${A}placeholder-product.svg'"><span>${esc(x.name||'مجموعة')}</span></button>`).join('')}</div>`:'<div class="empty-ux">لا توجد مجموعات</div>'
    }
    return '<div class="empty-ux">لا توجد عناصر في هذا القسم</div>'
  }
  function homeSections(){return secs().map(s=>`<section class="section spike-custom-section" data-section-id="${esc(s.id)}"><div class="title-row"><h2>${esc(s.title)}</h2>${s.show_all===false?'':`<button data-action="custom-section-more" data-id="${esc(s.id)}">عرض الكل</button>`}</div>${body(s,false)}</section>`).join('')}
  ...
  async function get(path){const sep=path.includes('?')?'&':'?';const r=await fetch(API+path+sep+'_='+Date.now(),{cache:'no-store',headers:{accept:'application/json','cache-control':'no-cache'}});const d=await r.json();if(!r.ok)throw Error(d.message||`${path} HTTP_${r.status}`);return d}
  async function loadDynamic(){try{const [ca,co,se]=await Promise.all([get('/categories'),get('/collections'),get('/home-sections')]);state.spikeContent=state.spikeContent||{};const currentCats=Array.isArray(state.spikeContent.spike_categories)?state.spikeContent.spike_categories:[];const liveCats=ca.categories||[];const mergedCats=[...liveCats,...currentCats.filter(old=>!liveCats.some(n=>String(n.id)===String(old.id)))];state.spikeContent.spike_categories=mergedCats;state.spikeContent.spike_collections=co.collections||[];state.spikeContent.home_sections=se.sections||[];state.liveCategories=mergedCats;state.liveCollections=co.collections||[];if(typeof render==='function')render()}catch(e){console.warn('Spike dynamic content',e)}}
  ...
  document.addEventListener('click',async e=>{
    ...
    if(a==='custom-section-more'){
      const id=el.dataset.id;try{const d=await get('/home-sections/'+id);const i=state.spikeContent.home_sections.findIndex(x=>String(x.id)===String(id));if(i>=0)state.spikeContent.home_sections[i]={...d.section,items:d.items||d.section?.items||[]};go(`custom-section:${id}`)}catch(err){showToast(err.message||'تعذر تحميل القسم')}
      ...
      else if(type==='store'){state.selectedStore=id;state.selectedStoreId=id;go('store-details')}
