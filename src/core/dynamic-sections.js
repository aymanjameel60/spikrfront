...
(function(){
  ...
  const API_ROOT='https://spike2.aymanjameel60.deno.net';
  ...
  const abs=v=>{const s=String(v||'').trim();if(!s)return '';if(/^https?:\/\//i.test(s))return s;if(s.startsWith('/uploads/'))return API_ROOT+s;if(s.startsWith('uploads/'))return API_ROOT+'/'+s;return typeof window.absoluteProductImage==='function'?window.absoluteProductImage(s):s};
  ...
  async function get(path){const r=await fetch(API+path,{cache:'no-store',headers:{accept:'application/json'}});const d=await r.json();if(!r.ok)throw Error(d.message||`${path} HTTP_${r.status}`);return d}
