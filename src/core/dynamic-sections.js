...
(function(){
  ...
  function normalizeProduct(p){if(!p)return null;if(typeof window.SpikeCommerce?.mapProduct==='function')return window.SpikeCommerce.mapProduct(p);const v=p.variants?.[0]||{};return{id:p.id,name:p.name,price:apiMoneyString(Number(v.price_usd||p.price_usd||0),'USD'),priceNumeric:Number(v.price_usd||p.price_usd||0),rating:String(p.review_average||0),img:abs(p.images?.[0]?.url||p.image_url||''),images:(p.images||[]).map(x=>abs(x.url||x)),store:p.store_name||'Spike',storeId:p.store_id,category:p.category_name||'',categoryId:p.category_id,variants:p.variants||[],selectedVariantId:v.id,inventory:Number(v.stock||0),currency:'USD',raw:p}}
